    ...
    <section id="doctors" className="py-20 md:py-28" style={{background: '#ffffff'}}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-16">
          <h2 className="font-heading text-3xl md:text-5xl font-bold mb-4" style={{color: '#18181b'}}>Meet Our <span className="gradient-text">Doctors</span></h2>
        </motion.div>
        <div className="grid md:grid-cols-3 gap-8">
          {doctors.map((d, i) => (
            <motion.div key={i} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.15 }}
              ...
              <div className="h-48 flex items-center justify-center" style={{background: 'linear-gradient(135deg, #18181b 0%, #27272a 50%, #71717a 100%)'}}>
              ...
              <div className="p-6">
                <h3 className="font-heading font-bold text-xl mb-1" style={{color: '#18181b'}}>{d.name}</h3>
                <p className="text-sm font-medium mb-2" style={{color: '#18181b'}}>{d.specialty}</p>
